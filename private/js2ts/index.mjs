...
const packageJSON = JSON.parse(await readFile(new URL('./package.json', packageRoot), 'utf-8'))

if (packageJSON.type !== 'module') {
  throw new Error('Cannot convert non-ESM package to TS')
}

const references = Object.keys(packageJSON.dependencies || {}).concat(Object.keys(packageJSON.peerDependencies || {}))
...
try {
  ...
  throw new Error('It seems this package has already been transitioned to TS', { cause })
}
...
for await (const dirent of dir) {
  if (!dirent.isDirectory()) {
    ...
    await writeFile(filepath.replace(ext, ext.replace('js', 'ts')),
...
await tsConfig.writeFile(`${JSON.stringify(
  {

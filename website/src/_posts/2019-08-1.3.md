...
Renee have attacked Dashboard performance head on, by making sure we avoid rerendering all the file items whenever something changes:
...
See [#1671](https://github.com/transloadit/uppy/pull/1671) for more details.

## Logger

We’ve added an option to supply `logger` object with `debug`, `warn` and `error` methods. Check out [the docs](https://uppy.io/docs/uppy/#logger) for more info.

In the future, past 2.0, `logger: Uppy.debugLogger` option is going to replace `debug: true`, the latter is kept for backwards compatability for now.
...
Before this change, when you added a file to Uppy from a mobile device, you’d sometimes see the preview image sideways. This was because we didn’t account for EXIF rotation metadata when generating thumbnails with `@uppy/thumbnail-generator`. But now, thanks to @Botz, we do! See [#1532](https://github.com/transloadit/uppy/pull/1661).

## File ID Generation

File ID generation has been adjusted to keep non-latin characters: now, non-latin characters are encoded as their charcode in base 32, so files that only differ by name in a non-latin language will generate different IDs. See [#1722](https://github.com/transloadit/uppy/pull/1722).

## Progress Improvements

* Show `total file size / total uploaded of all started` vs `total / total upload of those not complete` [#1685](https://github.com/transloadit/uppy/pull/1685).
* Fixed an issue where `_calculateTotalProgress` resulted in incorrectly high (1038%) progress with files that don’t have size (like from Instagram) [#1610](https://github.com/transloadit/uppy/pull/1610).
* We’ve moved from `prettier-bytes` package to `@uppy/utils/lib/prettyBytes`, and divide by `1024` instead of `1000` to justify `KB` vs `kB` [#1732](https://github.com/transloadit/uppy/pull/1732).

## Drag and Drop

* Drag and drop has been refactored internally in both Dashboard and DragDrop plugins. This means more robust and cross-browser implementation, with a fancy “drop files here” screen when you drag over the Dashboard.
...
* Renee have enabled matrix Travis CI, meaning we now run end to end and unit tests separate from website builds, for instance, which lets us identify issues with failing builds quciker, and also cutting down build times almost twice! 🚀  [#1709](https://github.com/transloadit/uppy/pull/1709)
* Webdriverio that we use for end to end testing has been updated to v5. This improves a bunch of things, and adds a lot of `await`s to our tests [#1675](https://github.com/transloadit/uppy/pull/1675).
...
* Changed cloud in gcloud-deploy (#1729 / @kiloreux)
...
* Properly loading instagram username, removed deprecated dropbox field #1651, #1692
...
* ⚠️ form: now excludes own metadata when creating a new upload. There’s also a new option to append result instead to input, instead of overwriting [#1686](https://github.com/transloadit/uppy/pull/1686).

...
As of Uppy 2.0.0, `Plugin` has been replaced with `BasePlugin` and `UIPlugin`. `BasePlugin` is the minimum you need to create a plugin and `UIPlugin` adds Preact for rendering user interfaces. If you are not using `UIPlugin` and don't want to bundle its dependencies (such as Preact) with your application, you can cherry-pick your imports to make sure `UIPlugin` doesn't end up in your bundle:
<!-- eslint-skip -->
```js
// UIPlugin may be bundled as it's exported by `@uppy/core`.
import { Uppy, BasePlugin } from '@uppy/core'
// Instead, you can import the classes individually.

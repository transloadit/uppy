...
Using React, Vue, or Svelte? The framework wrappers for this plugin were removed
in Uppy 5.0. Use the `Dropzone` component or `useDropzone` hook from
[`@uppy/react`](/docs/react), [`@uppy/vue`](/docs/vue), or
[`@uppy/svelte`](/docs/svelte) instead. This plugin remains supported for
vanilla JavaScript and no-build setups.

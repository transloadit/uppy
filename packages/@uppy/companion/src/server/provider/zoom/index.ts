  ...
  async list(options) {
    return this.#withErrorHandling('provider.zoom.list.error', async () => {
      ...
      const userRaw: unknown = await client
        ...
        .json()
      const user: Record<string, unknown> = isRecord(userRaw) ? userRaw : {}
      const timezone = typeof user.timezone === 'string' ? user.timezone : null
      ...
      return {
        username: typeof user.email === 'string' ? user.email : null,
        items: years,
        nextPagePath: null,
      }
    })

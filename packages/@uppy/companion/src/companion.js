...
const getOptionsMiddleware = (options) => {
  ...
  const middleware = (req, res, next) => {
    const versionFromQuery = req.query.uppyVersions
      ? decodeURIComponent(req.query.uppyVersions)
      : null;
    req.companion = {
      options,
      s3Client,
    ...
    logger.info(
      `uppy client version ${req.companion.clientVersion}`,
  ...
  if (providerOptions) {
    ...
    Object.keys(deprecatedOptions).forEach((deprected) => {
      if (providerOptions[deprected]) {
        throw new Error(
          `The Provider option "${deprected}" is no longer supported. Please use the option "${deprecatedOptions[deprected]}" instead.`
        );
      }
    });
  }

...
export class HTTPCommunicationQueue<M extends Meta, B extends Body> {
    ...
    async getUploadId(file: UppyFile<M, B>, signal: AbortSignal):Promise<UploadResult> {
      let cachedResult
    ...
    async uploadFile(file: UppyFile<M, B>, chunks: Chunk[], signal: AbortSignal): ReturnType<AwsS3Multipart<M, B>['completeMultipartUpload']> {

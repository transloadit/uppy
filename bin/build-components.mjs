...
async function generateWrappers() {
  try {
    ...
    await Promise.all([
      fs.access(COMPONENTS_DIR).catch(() => {
        throw new Error(`Components directory not found: ${COMPONENTS_DIR}`);
      }),
      fs.access(REACT_DIR).catch(() => fs.mkdir(REACT_DIR, { recursive: true })),
      fs.access(VUE_DIR).catch(() => fs.mkdir(VUE_DIR, { recursive: true })),
      fs.access(SVELTE_DIR).catch(() => fs.mkdir(SVELTE_DIR, { recursive: true })),
    ]);
    ...
    for (const file of tsxFiles) {
      try {
        ...
        const reactContent = REACT_TEMPLATE
        ...
        const svelteContent = SVELTE_TEMPLATE
          .replace(/%%ComponentName%%/g, componentName)
          .replace(/%%PreactComponentName%%/g, preactComponentName)
          .replace(/%%PropsTypeName%%/g, propsTypeName);
        ...
        console.log(`    - Svelte: ${svelteFilePath}`);
      } catch (error) {
